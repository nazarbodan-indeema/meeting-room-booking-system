...
import { checkIn } from '@/lib/actions/bookings';

interface CheckInClientProps {
  booking: any;
}

export function CheckInClient({ booking }: CheckInClientProps) {
  ...
  useEffect(() => {
    // Calculate real timeLeft based on booking.startTime
    const now = new Date();
    const startTime = new Date(booking.startTime);
    const diff = Math.floor((startTime.getTime() + 10 * 60 * 1000 - now.getTime()) / 1000);
      ...
      <div className="min-h-[60vh] flex items-center justify-center">
        ...
        >
          ...
          <p className="text-foreground-secondary mb-8">
            Your attendance for <strong>{booking.title}</strong> has been recorded.
          </p>
          <Link href="/bookings">
      ...
      <div className="grid md:grid-cols-2 gap-6 items-start">
            ...
            <div className="space-y-4 text-foreground-secondary mb-8">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm">Floor {booking.room?.floor}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm">{new Date(booking.startTime).toLocaleDateString()}</p>
                </div>
              </div>
            </div>
        ...
        >
          <Card padding="lg" className="text-center">
            ...
            <QRCode value={`BOOKING-${booking.id}`} className="mx-auto shadow-sm" />
